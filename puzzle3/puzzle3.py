...
class dungeon_game(game_board):
    def __init__(self):
        ...
        #don't go over the max number of turns!!!
        #because that's what morales does
        self.MAX_TURN_LIMIT = 10
    ...
    def _choose_and_move_actman(self):
        ...
        #good moveset wasn't found
            ...
            while depth_limit < self.MAX_TURN_LIMIT:
                #while this could be put as a while condition,
                #I think it would be better if it was a break condition so the while else
                #condition would still work for debugging
                if selected_option: break
                
                #print(f"Depth {depth_limit}")
                # Data structure to store stack of options for DFS
                stack = [{'initial_move': option, 'current_move': option, 'game_board': game_board(grid=self.grid, turn_count=0, points=self.points, moves='')} for option in valid_options]
                while stack:
                    #check
                    state: dict = stack.pop()
                    depth = state['game_board'].turn_count
                    ...
                    #did act man kill all the monsters (thereby winning)?
                        ...
                        break
                    #is act man still alive after 10 turns (i.e. 9 turns plus this one)?
                    #in case we aren't able to find a series of winning moves within 10 turns,
                    #get the last option where he survives 10 moves
                        ...
                        if not selected_option:
                            ...
                            if len(state['game_board'].moves) > 1:
                                self.moveset = state['game_board'].moves[1:]
                            selected_option = state['initial_move']
                    #Turn count for this move wasn't more than max_depth_limit, and it wasn't victory or defeat
                    ...
                    #get valid new options from this position, add to tail end of new options queue
                        ...
                        stack.extend(new_branch)
                
                #now that we've iterated at depth depth_limit, try the next depth limit
                depth_limit += 1
            ...
            #contingency if queue becomes empty and winning moveset not found
                ...
                if not selected_option:
                    print(f"No option found where actman survives another {self.MAX_TURN_LIMIT} turns. Moving randomly.")
